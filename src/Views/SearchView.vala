...
public class AppsView.Views.SearchView : Gtk.ScrolledWindow {
    ...
    Gee.HashMap<Widgets.SearchItem.ResultType, uint> limitator;
    ...
    construct {
        ...
        // list box
        limitator = new Gee.HashMap<Widgets.SearchItem.ResultType, uint> ();
        ...
        list_box.row_activated.connect ((row) => {
            Idle.add (() => {
                var search_item = row as Widgets.SearchItem;
                if (!list_box.dragging) {
                    switch (search_item.result_type) {
                        case Widgets.SearchItem.ResultType.APP_ACTIONS:
                        case Widgets.SearchItem.ResultType.LINK:
                        case Widgets.SearchItem.ResultType.SETTINGS:
    ...
    public void set_results (Gee.List<Synapse.Match> matches, string search_term) {
        ...
        if (matches.size > 0) {
            foreach (var match in matches) {
                ...
                Widgets.SearchItem.ResultType result_type = (Widgets.SearchItem.ResultType) match.match_type;
                if (match is Synapse.DesktopFilePlugin.ActionMatch) {
                    result_type = Widgets.SearchItem.ResultType.APP_ACTIONS;
                } else if (match is Synapse.SwitchboardObject) {
                    result_type = Widgets.SearchItem.ResultType.SETTINGS;
                } else if (match is Synapse.LinkPlugin.Result) {
                    result_type = Widgets.SearchItem.ResultType.INTERNET;
                }

                if (result_type == Widgets.SearchItem.ResultType.UNKNOWN) {
                    var actions = Backend.SynapseSearch.find_actions_for_match (match);
                    foreach (var action in actions) {
                        app = new Backend.App.from_synapse_match (action, match);
                        create_item (app, search_term, (Widgets.SearchItem.ResultType) app.match.match_type);
                    }
    ...
    private void create_item (Backend.App app, string search_term, Widgets.SearchItem.ResultType result_type) {
        ...
        var search_item = new Widgets.SearchItem (app, search_term, result_type);
    ...
    private int update_sort (Gtk.ListBoxRow row1, Gtk.ListBoxRow row2) {
        var item1 = row1 as Widgets.SearchItem;
        var item2 = row2 as Widgets.SearchItem;
    ...
    private void update_header (Widgets.SearchItem row, Widgets.SearchItem? before) {
